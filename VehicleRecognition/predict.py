...
def deskew(img):  # 抗扭斜，获取一个数字图像并将其校正，使用其二阶矩对图像进行去歪斜
    m = cv2.moments(img)
    if abs(m['mu02']) < 1e-2:
        return img.copy()
    skew = m['mu11'] / m['mu02']
...
# 来自opencv的sample，用于svm训练
def preprocess_hog(digits):  # 梯度方向直方图Histogram of Oriented Gradients (HOG)用于训练数据的特征向量
    samples = []
    for img in digits:
        ...
        hist = np.hstack(hists)  # hists是64位向量
...
class CardPredictor:
    ...
    def train_svm(self):
        ...
        if os.path.exists("svm.dat"):
            ...
            for root, dirs, files in os.walk("train\\chars2"):
                if len(os.path.basename(root)) > 1:  # os.path.basename返回文件名
                    continue
                root_int = ord(os.path.basename(root))  # ord()返回值是对应的十进制整数
    ...
    def predict(self, car_pic, resize_rate=1):
        ...
        if pic_width > MAX_WIDTH:
            ...
            # img = cv2.resize(img, (MAX_WIDTH, int(pic_height * resize_rate)), interpolation=cv2.INTER_LANCZOS4)
        if resize_rate != 1:
            img = cv2.resize(img, (int(pic_width * resize_rate), int(pic_height * resize_rate)),
                             interpolation=cv2.INTER_LANCZOS4)
            pic_height, pic_width = img.shape[:2]
        print("height,width:", pic_height, pic_width)
        # TODO 获取车牌更高效还有其它算法吗
        blur = self.cfg["blur"]
        # 高斯去噪
        if blur > 0:
        ...
        # Sobel_x = cv2.Sobel(img, cv2.CV_16S, 1, 0)  # sobel算子边缘检测
        ...
        print('len(contours):疑是轮廓个数', len(contours))

        # 一一排除不是车牌的矩形区域
        # TODO 排除高效尝试用cv2.boundingRect()
        ...
        # cv2.waitKey(0)
        print('len(car_contours):疑是车牌个数', len(car_contours))
        print("获取精确定位···")
        ...
        for rect in car_contours:
            ...
            if left_point[1] <= right_point[1]:  # 正角度
                ...
                M = cv2.getAffineTransform(pts1, pts2)  # 变形，仿射变换
                dst = cv2.warpAffine(oldimg, M, (pic_width, pic_height))  # warpAffine仿射变换
        ...
        for i, color in enumerate(colors):
            if color in ("blue", "yellow", "green"):
                ...
                x_threshold = (x_min + x_average) / 2  # TODO
                ...
                # 车牌字符数应大于5
                if len(wave_peaks) <= 5:
                    print("peak less 5:", len(wave_peaks))
                    continue
...
if __name__ == '__main__':
    ...
    print(r)
    # cv2.imwrite('vehicle/5_chepai.jpg', roi)
    # cv2.imshow('roi', roi)
    # print(type(roi))
    # print(color)
